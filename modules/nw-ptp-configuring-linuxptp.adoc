...
<5> Specify the network interface name to use by `ptp4l` service, for example `ens787f1`.
<6> Specify system config options for `ptp4l` service, for example `-s -2`. This should not include interface name `-i <interface>` and service config file `-f /etc/ptp4l.conf` which will be automatically appended.
<7> Specify system config options for `phc2sys` service, for example `-a -r`.
<8> Specify an array of one or more `recommend` objects which define rules on how `profile` should be applied to nodes.
<9> Specify the `profile` object name define in `profile` section.
<10> Specify the `priority` with an integer value between `0` and `99`. A larger number gets lower priority, so a priority of `99` is lower than a priority of `10`. If a node can be matched with multiple profiles according to rules defined in `match` field, profile with higher priority will be applied to that node.
<11> Specify `match` rules with `nodeLabel` or `nodeName`.
<12> Specify `nodeLabel` with the `key` of `node.Labels` from node object.
<13> Specify `nodeName` with `node.Name` from node object.
...
. Optional: Check that `PtpConfig` profile be applied to nodes that match with `nodeLabel` or `nodeName`.
+
----
...
----
<1>  `profile` name that is applied to node `dev-worker-0`.
<2> `Interface` is the PTP device specified in `profile1` interface field, `ptp4l` service runs on this interface.
<3> `Ptp4lOpts` is the ptp4l sysconfig options specified in `profile1` Ptp4lOpts field.
<4> `Phc2sysOpts` is the phc2sys sysconfig options specified in `profile1` Phc2sysOpts field.
